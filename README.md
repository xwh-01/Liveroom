...
直播间实时互动系统 MVP —— 基于 WebSocket 实现弹幕、礼物、排行榜、限流、在线人数统计功能。
...
确保 Redis 在 6379 端口运行。可修改 `backend/config/config.toml` 中的 Redis 地址。
...
| 版本 | 功能 |
|------|------|
| V1 | WebSocket + Redis 排行榜 + Redis 限流（当前） |
| V2 | MySQL 保存弹幕记录和礼物流水 |
| V3 | RabbitMQ 异步落库，解耦写入 |
| V4 | Prometheus + Grafana 监控在线人数、QPS、限流次数 |
| V5 | 微服务拆分：chat-service、gift-service、rank-service |
| V6 | 多实例 WebSocket，Redis Pub/Sub 跨节点广播 |
